...
\begin{itemize}[leftmargin=*]
    \item \textbf{10 Critical Vulnerabilities:} Authentication Bypass, SQL Injection, IDOR, XXE, Deserialization RCE, File Upload \& Overwrite, Race Condition, SSRF, Security Misconfiguration, and Maintenance Interface CSRF
...
\begin{itemize}[leftmargin=*]
    \item Implement 10 distinct security vulnerabilities with realistic exploitation scenarios
...
\subsection{Vulnerability 1: Authentication Bypass}
...
\subsection{Vulnerability 2: SQL Injection - Filter Bypass}

\subsubsection{Description}
Django ORM Q object abuse allowing OR-based filter injection. Admin users can bypass the default \texttt{is\_locked\_out=False} filter to reveal locked-out critical devices.
...
def vulnerable_dashboard(request):
    ...
    if is_admin:
        ...
        # Default: Admin only sees non-locked devices
        query = Q(is_locked_out=False)
        
        # VULNERABILITY: Admin can inject OR filter
        for key, value in filter_params.items():
            if connector == 'OR':
                query |= Q(**{key: value})  # DANGEROUS
            else:
                query &= Q(**{key: value})
...
# Step 2: Normal admin view (only unlocked devices)
curl http://localhost:8000/vulnerable/dashboard/ -b cookies.txt
# Shows: 18 operational devices (no NUCLEAR)

# Step 3: SQL Injection - reveal locked-out devices
curl "http://localhost:8000/vulnerable/dashboard/?connector=OR&is_locked_out=True" -b cookies.txt
# Shows: 21 devices including NUCLEAR-CORE-CONTROLLER (locked-out)
\end{lstlisting}

\textbf{Attack Logic:}\\
Original query: \texttt{WHERE is\_locked\_out=False}\\
Injected query: \texttt{WHERE is\_locked\_out=False OR is\_locked\_out=True}\\
Result: All devices revealed (bypasses safety lock)

\textbf{Impact:}
\begin{itemize}[leftmargin=*]
    \item Reveals locked-out critical infrastructure (NUCLEAR reactor)
    \item Bypasses safety lockout mechanisms
    \item Only works for admin users (regular users have hardcoded filter)
    \item Enables reconnaissance for further attacks
\end{itemize}

\textbf{Why Only Admin is Vulnerable:}
This design reflects real-world scenarios where:
\begin{itemize}[leftmargin=*]
    \item Admin panels contain complex filtering/search features
    \item Developers trust admin users and add "power user" functionality
    \item Regular users get simple, hardcoded, secure views
    \item Security is overlooked in privileged interfaces ("admin is trusted")
    \item Demonstrates vulnerability chaining: Auth Bypass (become admin) + SQL Injection (reveal secrets)
\end{itemize}
...
\subsection{Vulnerability 5: File Upload \& Overwrite}
...
\subsection{Vulnerability 7: Race Condition in File Generation}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Automated Tool:} Burp Suite
