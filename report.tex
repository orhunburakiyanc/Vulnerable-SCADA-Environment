...
\subsubsection{Description}
Django ORM Q object abuse allowing OR-based filter injection. Admin users can bypass the default \texttt{is\_locked\_out=False} filter to reveal locked-out critical NUCLEAR devices.
...
\subsection{Vulnerability 4: IDOR - Insecure Direct Object Reference (Vulnerability A)}
...
\subsection{Vulnerability 5: Insecure Deserialization (Pickle RCE) - Vulnerability B}
...
\subsection{Vulnerability 6: File Upload \& Overwrite (Vulnerabilities C \& E)}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Input validation and sanitization
\end{itemize}
8: Race Condition in File Generation (Vulnerability D - Unsafe Temp Files)}

\subsubsection{Description}
Race condition vulnerability (CWE-377) due to predictable temporary file paths that are reused across users.
\subsubsection{Description}
Static temporary filename allows concurrent requests to overwrite each other's data, causing race condition vulnerabilities.
...
\end{itemize}9: SSRF - Server-Side Request Forgery (Vulnerability F)}

\subsubsection{Description}
Server-Side Request Forgery vulnerability (CWE-918) - Application accepts arbitrary URLs and makes server-side requests without validation, allowing remote access to internal systems
\subsubsection{Description}
...
\newpage
1
\subsection{Vulnerability 10: SCADA Maintenance Mode Interface - Multiple Security Flaws (OWASP A01:2021 + A05:2021 + A07:2021)}
...
Open-source web application security scanner used for automated testing:
\begin{itemize}[leftmargin=*]
    \item \textbf{Spider:} Automated crawling of vulnerable and patched endpoints
    \item \textbf{Active Scanner:} Automated detection of:
    \begin{itemize}
        \item SQL Injection (CVE-2025-64459)
        \item Authentication Bypass vulnerabilities
        \item CSRF vulnerabilities
        \item Server-Side Request Forgery (SSRF)
        \item XXE (XML External Entity) injection
        \item Information Disclosure (DEBUG mode)
    \end{itemize}
    \item \textbf{Fuzzer:} Input validation testing for file upload and deserialization endpoints
    \item \textbf{HUD (Heads Up Display):} Real-time security analysis during manual testing
\end{itemize}

\textbf{ZAP Configuration for Testing:}
\begin{lstlisting}[language=bash]
# Quick scan of vulnerable application
zap.sh -quickurl http://localhost:8000/vulnerable/ -quickout zap_report.html

# Active scan with authentication context
zap-cli quick-scan --self-contained --start-options "-config api.key=changeme" \
  http://localhost:8000/vulnerable/login/

# API scan
zap-cli active-scan http://localhost:8000/vulnerable/dashboard/
\end{lstlisting}

\textbf{Detected by ZAP:}
\begin{itemize}[leftmargin=*]
    \item ✓ SQL Injection in dashboard filter (connector=OR parameter)
    \item ✓ Authentication bypass via GET parameters (is\_admin=true)
    \item ✓ CSRF tokens missing on maintenance actions
    \item ✓ SSRF vulnerability in node check endpoint
    \item ✓ XXE vulnerability in XML upload parser
    \item ✓ Information Disclosure (DEBUG=True, detailed error pages)
    \item ✓ Insecure Direct Object Reference (IDOR) in report downloads
\end{itemize}
